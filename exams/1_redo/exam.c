...
unsigned char MirrorByte2(unsigned char byte) /* bits that fall off dont have to be masked to 0 */
{
	byte = (byte << 4) | (byte >> 4);
	byte = (((byte & 0xf3) << 2) | ((byte & 0xcf) >> 2));
	return (((byte & 0xd5) << 1) | ((byte & 0xab) >> 1));
}

unsigned char MirrorByte3(unsigned char byte) /* shift then mask can also work (opposite masks from original) */
{
	byte = (byte << 4) | (byte >> 4);
	byte = (((byte << 2) & 0xcc) | ((byte >> 2) & 0x33));
	return (((byte << 1) & 0xaa) | ((byte >> 1) & 0x55));
}
